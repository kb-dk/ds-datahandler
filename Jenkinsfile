                ...
                stage('Create test project') {
                    ...
                    openshift.withProject(projectName) {
                        ...
                        stage("Create build and deploy application") {
                            openshift.newBuild("--strategy source", "--binary", "-i kb-infra/kb-s2i-tomcat90", "--name ds-datahandler")
                            openshift.startBuild("ds-datahandler", "--from-dir=.", "--follow")
                            openshift.newApp("ds-datahandler:latest")
                            openshift.create("route", "edge", "--service=ds-datahandler")
                        }
                    }
                }
